...
void para_check_print(para* p, para* q);
...
void para_check_print(para* p, para* q){
  ...
  for (int i = p->start,j = q->start; i <= p->stop && i != p->filesize&&j <= q->stop && j != q->filesize; ++i,++j) {
    if(compare_buf(p->base[i],q->base[j])){
      printboth(p->base[i]);
    }else{
      checkprint(p->base[i],q->base[j]);
    }
  }
}

void para_check_left_print(para*p, para* q){
  if (p == NULL||q==NULL) { return; }
  for (int i = p->start,j = q->start; i <= p->stop && i != p->filesize&&j <= q->stop && j != q->filesize; ++i,++j) {
    if(compare_buf(p->base[i],q->base[j])){
      leftcheckprint(p->base[i]);
    }else{
      checkprint(p->base[i],q->base[j]);
    }
  }
...
void para_suppress_print(para* p, para* q){
  if (p == NULL||q==NULL) { return; }
  for (int i = p->start,j = q->start; i <= p->stop && i != p->filesize&&j <= q->stop && j != q->filesize; ++i,++j) {
    if(!compare_buf(p->base[i],q->base[j])){
      checkprint(p->base[i],q->base[j]);
