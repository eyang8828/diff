...
//  para_printfile(strings2, count2, printright);
  ...
  para* lp = para_first(strings1, count1);
  para* rp = para_first(strings2, count2);
  int foundmatch = 0;
  ...
  while (lp != NULL) {
    rplast = rp;
    foundmatch = 0;
    while (rp != NULL && (foundmatch = para_equal(lp, rp)) == 0) {
      rp = para_next(rp);
    }
    rp = rplast;

    if (foundmatch) {
      while ((foundmatch = para_equal(lp, rp)) == 0) {
        para_print(rp, printright);
        rp = para_next(rp);
        rplast = rp;
      }
      if(suppresscommon){
        para_suppress_print(lp,rp);
      }else if(showsidebyside){
        if(showleftcolumn){
          para_check_left_print(lp,rp);
        }else{
          para_check_print(lp,rp);
        }
      }else{
        para_print(rp, printboth);
      }
      lp = para_next(lp);
      rp = para_next(rp);
    } else {
      para_print(lp, printleft);
      lp = para_next(lp);
    }
  }
  while (rp != NULL) {
